...
from ..database import get_db, JobPhase
from ..schemas import (
    JobResponse, JobDetail, QuestionListResponse, QuestionSchema,
    QuestionUpdate, SelectQuestionsRequest, VariationTriggerRequest, ErrorResponse,
)
...
async def create_job(
    ...
    try:
        job = job_service.create_job(db, file.filename, tmp_path, custom_instruction)
    finally:
        os.unlink(tmp_path)
...
async def get_questions(job_id: str, db: Session = Depends(get_db)):
    job = job_service.get_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
async def get_question(job_id: str, qid: str, db: Session = Depends(get_db)):
    job = job_service.get_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
async def update_question(
    ...
    updated = job_service.update_question(db, job, qid, updates)
    if not updated:
        raise HTTPException(status_code=404, detail="Question not found")
...
async def select_question(job_id: str, qid: str, db: Session = Depends(get_db)):
    job = job_service.get_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    ...
    for i, q in enumerate(questions):
        if q.get("question_id") == qid:
            if i not in selected:
                selected.append(i)
                job_service.select_questions(db, job, selected)
...
async def bulk_select_questions(
    job_id: str,
    ...
    job_service.select_questions(db, job, req.selected_indices)
    return _job_response(job)
...
async def trigger_variations(
    ...
    if job.phase not in (JobPhase.SOLVED, JobPhase.COMPLETED, JobPhase.FAILED):
        raise HTTPException(status_code=409, detail=f"Job is in phase '{job.phase}', not ready for variations")

    if req.custom_instruction:
        job_service.update_job(db, job, custom_instruction=req.custom_instruction)

    background_tasks.add_task(extraction_service.run_variations, job.id)
    return {"status": "variations_queued", "job_id": job_id}


@router.get("/{job_id}/variations")
async def get_variations(job_id: str, db: Session = Depends(get_db)):
    job = job_service.get_job(db, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")

    from ..schemas import VariationResultSchema, VariationSchema, QuestionSchema
...
async def trigger_export(
    ...
    if not job.variations_json:
        raise HTTPException(status_code=409, detail="No variations generated yet")

    background_tasks.add_task(extraction_service.run_export, job.id)
...
def _job_response(job) -> JobResponse:
    ...
    return JobResponse(
        ...
        phase=JobPhase(job.phase.value) if isinstance(job.phase, JobPhase) else job.phase,
